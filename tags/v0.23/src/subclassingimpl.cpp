...
void SubclassingImpl::on_okButton_clicked()
{
    ...
    {
        if ( ((Editor *)m_mainImpl->tabEditors()->widget(i))->filename() == filename+ ".h" )
        {
            editorHeader = ((Editor *)m_mainImpl->tabEditors()->widget(i));
        }
    }
    //
    if ( editorHeader )
    {
        ...
        headerImpl = editorHeader->toPlainText().split("\n");
    }
    else
    {
    	// If file exists, get content from file
        if (file.open(QIODevice::ReadOnly | QIODevice::Text))
        {
            headerImpl = QString(file.readAll()).split("\n");
            file.close();
        }
        else // Otherwise creation of a new content from template
            headerImpl = templateHeaderImpl();
    }
    //
    int headerInsertLineAfter = headerImpl.indexOf( "private slots:" );
    if ( headerInsertLineAfter != -1 )
    {
        if ( editorHeader )
        {
            headerInsertLineAfter += 2;
        }
        else
        {
            headerInsertLineAfter += 1;
        }
    }
    if ( headerInsertLineAfter == -1 )
    {
    ...
    for (int i=0; i<m_mainImpl->tabEditors()->count(); i++)
    {
        if ( ((Editor *)m_mainImpl->tabEditors()->widget(i))->filename() == filename+ ".cpp" )
        {
            editorImplementation = ((Editor *)m_mainImpl->tabEditors()->widget(i));
        }
    }
    ...
    // Now insert code for all new items checked 
        ...
        for (int j=0; j< topLevelItem->childCount(); j++)
        {
            ...
            if ( item->checkState( 0 ) && item->textColor(0)!= Qt::blue )
            {
                ...
                if ( editorHeader )
                    ...
                    headerImpl.insert(headerInsertLineAfter++, "\tvoid "+s+";");
                if ( editorImplementation )
                {
                    editorImplementation->insertText("void "+ comboClassName->currentText() + "::" + s, -1);
                    editorImplementation->insertText("{", -1);
                    editorImplementation->insertText("\t// TODO", -1);
                    editorImplementation->insertText("}\n", -1);
