	...
	: QDialog(0), m_projectManager(parent), m_itProject(itProject), m_treeFiles(tree)
{
	...
	setWindowTitle( tr("Properties of")+" "+m_copyItProject->text(0) );
	populateComboScope();
...
void ProjectPropertieImpl::slotComboScope(int index)
{
	...
	for(int nbScope=0; nbScope < item->childCount(); nbScope++)
	{
		...
		if( !QString("CONFIG|FORMS|HEADERS|QT|RESOURCES|SOURCES|TRANSLATIONS|TEMPLATE|SCOPE|SUBDIRS|absoluteNameProjectFile|projectDirectory|subProjectName|qmake").contains( cle ) )
		{
