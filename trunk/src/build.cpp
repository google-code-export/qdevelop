...
#ifdef Q_OS_WIN32
    QString makePath = QFileInfo(m_makeName).absoluteDir().absolutePath();
    QString qmakePath = QFileInfo(m_qmakeName).absoluteDir().absolutePath();
    QStringList env = QProcess::systemEnvironment();
    env.replaceInStrings(QRegExp("^PATH=(.*)", Qt::CaseInsensitive), "PATH=\\1;"+makePath+";"+qmakePath);
    m_buildProcess->setEnvironment(env);
#endif
